...
//!
//! CF semantics pinned here (Litz tall-target convention as specced):
//!   * dial-unit outputs (mil/moa/smoa/iphy/clicks) are MULTIPLIED by the axis's CF
//!     exactly once, at the shared conversion boundary — raw drop/drift inches never;
//!   * truing `--observed`/`--measured-drop` dialed values are DIVIDED by the CF before
//!     the fit consumes them (CF=1.1, dialed 11 -> the fit sees 10), and dial-unit
//!     report values are shown back in scope units;
...
fn cf_of_one_is_byte_identical() {
    let cases: Vec<(Vec<&str>, Vec<&str>)> = vec![
        ...
        (
            [&["true-velocity"], TV_LOAD, &["--range", "600", "--measured-drop", "4.1"]].concat(),
            vec!["--elevation-cf", "1.0"],
        ),
        (
...
// (b) CF = 1.05 scales every dial surface's values by exactly 1.05.
#[test]
fn cf_scales_every_dial_surface_by_exactly_1_05() {
    let close = |scaled: f64, base: f64, what: &str| {
        assert!(
            (scaled - base * 1.05).abs() <= 1e-9 * base.abs().max(1.0),
            "{what}: {scaled} != {base} * 1.05"
        );
    };
    ...
    cf_args.extend(["--elevation-cf", "1.05"]);
    ...
    let mut cf_args = base_args.clone();
    cf_args.extend(["--elevation-cf", "1.05", "--windage-cf", "1.05"]);
    ...
    let mut cf_args = base_args.clone();
    cf_args.extend(["--windage-cf", "1.05"]);
    let (b, s) = (json_ok(&base_args), json_ok(&cf_args));
    for (rb, rs) in b["data"].as_array().unwrap().iter().zip(s["data"].as_array().unwrap()) {
...
// (b2) clicks scale as whole clicks of the scaled angle.
#[test]
fn cf_scales_clicks_through_the_quantizer() {
    let base_args = [
        &["come-ups"][..],
        LOAD,
        &[
    ...
    // The unscaled mil values (same run, mil unit) predict the scaled click counts:
    // clicks = round(mil * 1.05 / 0.1).
    let mil_args = [
        &["come-ups"][..],
        LOAD,
    ...
    {
        let expected = (rm["drop"].as_f64().unwrap() * 1.05 / 0.1).round();
        assert_eq!(
...
// (c) truing division direction: CF=1.1, dialed 11 -> the fit sees 10.
#[test]
fn truing_divides_dialed_observations_by_the_cf() {
    // Single observation: identical fitted velocity to a 10-mil run without a CF.
    let with_cf = [
        &["true-velocity"][..],
        TV_LOAD,
        &["--range", "600", "--measured-drop", "11", "--elevation-cf", "1.1", "-o", "json"],
    ]
    .concat();
    let without = [
        &["true-velocity"][..],
        TV_LOAD,
        &["--range", "600", "--measured-drop", "10", "-o", "json"],
    ]
    .concat();
    let (cf_doc, base_doc) = (json_ok(&with_cf), json_ok(&without));
    // 11/1.1 can differ from 10.0 by an ulp, and the bisection's bracket collapses at
    // 0.5 fps — 1 fps still pins the direction unambiguously (a fit of 11 dialed mils
    // without the division lands hundreds of fps away).
    let (v_cf, v_base) = (
    ...
    assert!(
        (v_cf - v_base).abs() <= 1.0,
        "the fit must consume 11 / 1.1 = 10 true mils: {v_cf} vs {v_base}"
    );
    // ...while the echo shows what the shooter dialed (scope units).
    let echoed = cf_doc["measured_drop_mil"].as_f64().unwrap();
    assert!((echoed - 11.0).abs() < 1e-9, "echo must stay 11 dialed mils, got {echoed}");
    // and the model's drop renders in scope units too: base calculated * 1.1.
    let (calc_cf, calc_base) = (
        cf_doc["calculated_drop_mil"].as_f64().unwrap(),
        base_doc["calculated_drop_mil"].as_f64().unwrap(),
    );
    assert!(
        (calc_cf - calc_base * 1.1).abs() < 1e-3,
        "calculated drop must render in scope units: {calc_cf} vs {calc_base} * 1.1"
    );

    // Multi-observation: dividing every dialed drop reproduces the un-CF'd fit.
    let with_cf = [
        &["true-velocity"][..],
        TV_LOAD,
        &[
            "--range", "600", "--measured-drop", "4.51", "--observed", "800:8.69",
            "--elevation-cf", "1.1", "-o", "json",
        ],
    ]
    .concat();
    let without = [
    ...
    assert!(
        ...
        "multi-obs fit must consume the divided (true) drops: {mv_cf} vs {mv_base}"
    );
    let (bc_cf, bc_base) = (
    ...
    let with_cf = [
        ...
        &[
            ...
            "--drop-unit", "in", "--elevation-cf", "1.1", "-o", "json",
        ],
    ]
    .concat();
    let without = [
...
fn validation_rejects_out_of_band_factors() {
    ...
    assert!(
        (ds - df * 1.1).abs() < 1e-9,
        "stored CF 1.1 must apply ({ds}) and the 1.0 flag must override it ({df})"
    );
}
