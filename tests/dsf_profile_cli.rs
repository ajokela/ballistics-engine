...
fn pick_subsonic_point(traj: &serde_json::Value) -> (f64, f64) {
    let points = traj["trajectory"]
        .as_array()
        .expect("trajectory array (need --full)");
    for p in points {
...
fn dsf_verb_derives_saves_and_trajectory_auto_applies() {
    ...
    assert!(
        dsf_stdout.contains("DSF table for 'dsf-roundtrip'"),
        "{dsf_stdout}"
    );
    ...
    assert!(
        (dsf_ratio - 1.3).abs() < 0.05,
        "dsf ratio should be ~1.3: {dsf_ratio}"
    );
    ...
    for (bp, cp) in base_points.iter().zip(corr_points.iter()) {
        ...
        assert!(
            (bv - cv).abs() < 1e-9,
            "per-point velocity must stay identical: {bv} vs {cv}"
        );
        let bz = bp["z"].as_f64().unwrap();
        let cz = cp["z"].as_f64().unwrap();
        assert!(
            (bz - cz).abs() < 1e-9,
            "per-point downrange must stay identical: {bz} vs {cz}"
        );
    ...
    assert!(
        !corrected_raw.to_lowercase().contains("dsf"),
        "{corrected_raw}"
    );
    ...
    assert!(
        table_stdout.contains("DSF table active (1 points"),
        "{table_stdout}"
    );
    ...
    assert!(
        cleared.get("dsf_points").is_none(),
        "dsf_points key must be gone: {cleared}"
    );
...
fn old_profile_without_dsf_points_key_loads_clean() {
    ...
    assert!(
        traj.status.success(),
        "{}",
        String::from_utf8_lossy(&traj.stderr)
    );
...
fn beyond_90pct_warning_requires_both_gates_not_just_range_ratio() {
    ...
    assert!(
        mid.status.success(),
        "{}",
        String::from_utf8_lossy(&mid.stderr)
    );
    ...
    assert!(
        far.status.success(),
        "{}",
        String::from_utf8_lossy(&far.stderr)
    );
...
fn come_ups_auto_applies_the_dsf_table_with_a_table_only_note() {
    ...
    for (b, c) in base_rows.iter().zip(corr_rows.iter()) {
        ...
        assert!(
            (bv - cv).abs() < 1e-6,
            "velocity must stay identical: {bv} vs {cv}"
        );
        let bt = b["time"].as_f64().unwrap();
        let ct = c["time"].as_f64().unwrap();
        assert!(
            (bt - ct).abs() < 1e-6,
            "time must stay identical: {bt} vs {ct}"
        );
    ...
    assert!(
        any_drop_changed,
        "at least one row's drop must change after DSF auto-apply"
    );
    ...
    assert!(!String::from_utf8_lossy(&csv.stdout)
        .to_lowercase()
        .contains("dsf"));
}
