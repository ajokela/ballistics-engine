...
ssh "$H" "set -e; source ~/.cargo/env
  ...
  cd /opt/mips-build/ballistics-engine* 2>/dev/null || { git clone https://github.com/ajokela/ballistics-engine /opt/mips-build/ballistics-engine && cd /opt/mips-build/ballistics-engine; }
