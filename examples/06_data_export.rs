...
fn export_gnuplot(trajectory: &[TrajectoryPoint], filename: &str) -> std::io::Result<()> {
    ...
    for point in trajectory {
        writeln!(
            file,
            "{:.4} {:.3} {:.3} {:.3} {:.2} {:.2} {:.3}",
            point.time, point.x, point.y, point.z, point.velocity, point.energy, point.mach
        )?;
    }
...
fn export_ballistic_table(trajectory: &[TrajectoryPoint], filename: &str) -> std::io::Result<()> {
    ...
    writeln!(
        file,
        " Range | Drop  | Drift | Time  | Velocity | Energy | Mach"
    )?;
    writeln!(
        file,
        "  (m)  |  (m)  |  (m)  | (s)   |  (m/s)   |  (J)   |  #"
    ...
    for target_range in ranges {
        ...
        if let Some(point) = trajectory.iter().find(|p| p.x >= target_range) {
            writeln!(
                file,
                " {:5.0} | {:5.2} | {:5.2} | {:5.3} | {:8.1} | {:6.0} | {:.2}",
                point.x, -point.y, point.z, point.time, point.velocity, point.energy, point.mach
            )?;
        }
    }
...
fn create_gnuplot_script(filename: &str) -> std::io::Result<()> {
    ...
    writeln!(
        file,
        "plot 'trajectory.dat' using 2:3 with lines title 'Trajectory' lw 2"
    )?;
    writeln!(file)?;
    ...
    writeln!(
        file,
        "plot 'trajectory.dat' using 2:5 with lines title 'Velocity' lw 2"
    )?;
    writeln!(file)?;
    ...
    writeln!(
        file,
        "plot 'trajectory.dat' using 1:6 with lines title 'Energy' lw 2"
    )?;
    writeln!(file)?;
    ...
    writeln!(
        file,
        "plot 'trajectory.dat' using 2:7 with lines title 'Mach' lw 2"
    )?;
    writeln!(file)?;
...
fn create_python_plot_script(filename: &str) -> std::io::Result<()> {
    ...
    writeln!(
        file,
        "ax2.plot(data['x'], data['velocity'], 'r-', linewidth=2)"
    )?;
    ...
    writeln!(
        file,
        "ax3.plot(data['time'], data['energy'], 'g-', linewidth=2)"
    )?;
    ...
    writeln!(
        file,
        "ax4.axhline(y=1.0, color='k', linestyle='--', alpha=0.5, label='Mach 1')"
    )?;
