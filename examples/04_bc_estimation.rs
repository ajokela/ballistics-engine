...
fn main() {
    ...
    for (dist, observed_drop) in &observed_data {
        let calculated_drop =
            calculate_drop_at_distance(velocity, mass, diameter, estimated_bc, *dist);
    ...
    for bc in bc_values {
        ...
        let marker = if (bc - estimated_bc).abs() < 0.01 {
            " <--"
        } else {
            ""
        };
...
fn estimate_bc_iterative(
    ...
    for bc_test in (100..1000).step_by(5) {
        ...
        let mut total_error = 0.0;
        for (dist, observed_drop) in observed_data {
            let calculated_drop = calculate_drop_at_distance(velocity, mass, diameter, bc, *dist);
...
fn estimate_bc_least_squares(
    ...
    for _ in 0..iterations {
        ...
        for (dist, observed_drop) in observed_data {
            ...
            let bc_delta = 0.001;
            let drop_plus =
                calculate_drop_at_distance(velocity, mass, diameter, bc + bc_delta, *dist);
