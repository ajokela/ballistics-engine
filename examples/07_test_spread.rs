use ballistics_engine::{
    run_monte_carlo_with_wind, BallisticInputs, DragModel, MonteCarloParams, WindConditions,
};
...
fn main() {
    ...
    match run_monte_carlo_with_wind(base_inputs, base_wind, mc_params) {
        Ok(results) => {
            ...
            println!("\nImpact spread:");
            println!(
                "X (range) spread: {:.2} to {:.2} m (spread: {:.2} m)",
                x_min,
                x_max,
                x_max - x_min
            );
            println!(
                "Z (lateral) spread: {:.2} to {:.2} m (spread: {:.2} m)",
                z_min,
                z_max,
                z_max - z_min
            );
