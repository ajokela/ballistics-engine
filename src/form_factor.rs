...
pub fn get_default_form_factor(bullet_name: &str, drag_model: &DragModel) -> f64 {
    ...
    match drag_model {
        DragModel::G1 => {
            if name_upper.contains("MATCH")
                || name_upper.contains("SMK")
                || name_upper.contains("SCENAR")
                || name_upper.contains("BERGER")
            {
                0.90 // Match bullets
            } else if name_upper.contains("VLD")
                || name_upper.contains("HYBRID")
...
mod tests {
    ...
    fn test_apply_form_factor() {
        ...
        let cd_with_ff = apply_form_factor_to_drag(cd_base, Some("168gr SMK"), &drag_model, true);
        assert_eq!(cd_with_ff, 0.5 * 0.90); // 0.45

        // With form factor disabled
        let cd_without_ff =
            apply_form_factor_to_drag(cd_base, Some("168gr SMK"), &drag_model, false);
        assert_eq!(cd_without_ff, 0.5);

        // No bullet name
        let cd_no_name = apply_form_factor_to_drag(cd_base, None, &drag_model, true);
