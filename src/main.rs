...
enum Commands {
    ...
    /// Compute a scope tracking correction factor from a tall-target test (MBA-1358):
    /// CF = actual measured travel / dialed travel. Pure arithmetic — no trajectory
    /// solve. Feed the result to --elevation-cf/--windage-cf or store it on a profile
    /// (elevation_cf/windage_cf).
    TallTarget {
        /// Dialed travel in --unit (e.g. 10 for a 10 mil tall-target test)
        #[arg(long, value_name = "DIAL")]
        dialed: f64,

        /// Actual measured group travel on the target, inches (imperial) or cm
        /// (metric, with --units metric)
        #[arg(long, value_name = "TRAVEL")]
        measured: f64,

        /// Distance to the tall target, yards (imperial) or meters (metric)
        #[arg(long, value_name = "RANGE")]
        range: f64,

        /// Angular unit the turret was dialed in (mil, moa, smoa, or iphy)
        #[arg(long, value_enum, default_value = "mil")]
        unit: AdjustmentUnit,
    },
    ...
    /// (MBA-1372): ejecta (bullet) + propellant gas recoil against the firearm.
    /// See CLI_USAGE.md for the gas-velocity convention this command defaults to.
    Recoil {
        /// Bullet weight (grains for imperial, grams for metric)
        #[arg(short = 'b', long, value_parser = f64_range(0.1, 2000.0))]
        bullet_weight: f64,

        /// Powder charge weight (grains for imperial, grams for metric)
        #[arg(short = 'c', long, value_parser = f64_range(0.0, 1000.0))]
        charge_weight: f64,

        /// Muzzle velocity (fps or m/s based on --units)
        #[arg(short = 'v', long, value_parser = f64_range(1.0, 6000.0))]
        velocity: f64,

        /// Firearm weight, including all attachments such as scopes and suppressors
        /// (pounds for imperial, kilograms for metric)
        #[arg(short = 'f', long, value_parser = f64_range(0.1, 500.0))]
        firearm_weight: f64,

        /// Firearm type, selecting SAAMI's propellant-gas-velocity multiplier f
        /// (gas velocity = f * muzzle velocity): rifle (f=1.75), pistol (f=1.50),
        /// shotgun-average (f=1.50), shotgun-long (f=1.25). Ignored if
        /// --gas-velocity-factor or --gas-velocity is given.
        #[arg(long, default_value = "rifle")]
        firearm_type: RecoilFirearmTypeArg,

        /// Override the SAAMI type-based multiplier with an explicit ratio
        /// (gas velocity = factor * muzzle velocity). Takes precedence over
        /// --firearm-type; overridden by --gas-velocity if both are given.
        #[arg(long, value_parser = f64_range(0.0, 20.0))]
        gas_velocity_factor: Option<f64>,

        /// Override with an absolute propellant gas velocity (fps or m/s based on
        /// --units), independent of muzzle velocity -- e.g. the classic fixed
        /// "~4700 fps for smokeless powder" rule of thumb. Takes precedence over
        /// both --gas-velocity-factor and --firearm-type.
        #[arg(long, value_parser = f64_range(0.0, 20000.0))]
        gas_velocity: Option<f64>,
    ...
    /// Power factor (bullet weight x velocity / 1000) with per-organization
    /// USPSA/IDPA/SASS rulebook pass/fail (MBA-1372). See CLI_USAGE.md for the
    /// truncation convention and threshold boundary semantics.
    PowerFactor {
        /// Bullet weight (grains for imperial, grams for metric)
        #[arg(short = 'w', long, value_parser = f64_range(0.1, 2000.0))]
        weight: f64,

        /// Velocity (fps or m/s based on --units)
        #[arg(short = 'v', long, value_parser = f64_range(1.0, 6000.0))]
        velocity: f64,

        /// Filter results to one organization: uspsa, idpa, or sass (default: all)
        #[arg(long)]
...
/// type-keyed propellant-gas-velocity multiplier for `recoil --firearm-type` (MBA-1372).
/// Kept as a thin clap `ValueEnum` here (rather than deriving `ValueEnum` on the lib enum
/// itself) so `recoil.rs` stays clap-free and trivially compiles for wasm32, matching the
/// established `WindShearModelArg` pattern.
#[derive(Debug, Clone, Copy, PartialEq, ValueEnum, Default)]
enum RecoilFirearmTypeArg {
    #[default]
    Rifle,
    Pistol,
    #[value(name = "shotgun-average", alias = "shotgun")]
    ShotgunAverage,
    #[value(name = "shotgun-long")]
    ShotgunLong,
}

impl RecoilFirearmTypeArg {
    fn to_engine(self) -> FirearmType {
        match self {
            RecoilFirearmTypeArg::Rifle => FirearmType::Rifle,
            RecoilFirearmTypeArg::Pistol => FirearmType::Pistol,
            RecoilFirearmTypeArg::ShotgunAverage => FirearmType::ShotgunAverage,
            RecoilFirearmTypeArg::ShotgunLong => FirearmType::ShotgunLong,
        }
    }
}
...
fn load_profile(name: &str) -> Result<ProfileData, Box<dyn Error>> {
    ...
    // MBA-1358: tracking correction factors are validated ON LOAD (hard error naming the
    // field) so a hand-edited or corrupted CF can never silently scale a firing solution.
    if let Some(cf) = profile.elevation_cf {
        validate_tracking_cf(cf, "profile field elevation_cf")?;
    }
    if let Some(cf) = profile.windage_cf {
        validate_tracking_cf(cf, "profile field windage_cf")?;
    }
    Ok(profile)
}
...
/// `ballistics_engine::adjustment::tracking_cf_in_range` so the WASM terminal enforces
/// the identical bound.
fn validate_tracking_cf(value: f64, source: &str) -> Result<(), String> {
    if ballistics_engine::adjustment::tracking_cf_in_range(value) {
        Ok(())
    } else {
        Err(format!(
...
/// keeps this the single resolution point).
fn resolve_tracking_cf(
    flag: Option<f64>,
    profile_value: Option<f64>,
    flag_name: &str,
) -> Result<f64, String> {
    let value = flag.or(profile_value).unwrap_or(1.0);
    validate_tracking_cf(value, flag_name)?;
    Ok(value)
}
    ...
    match cli.command {
        ...
        Commands::Trajectory {
            ...
            let sight_offset_lateral_metric = sight_offset
                ...
                .unwrap_or(0.0);
            // MBA-1358: scope tracking CFs — CLI flag over profile field, validated here
            // (and again on profile load); 1.0 = no correction, byte-identical.
            let elevation_cf = resolve_tracking_cf(
                elevation_cf,
                saved_profile_data.as_ref().and_then(|p| p.elevation_cf),
                "--elevation-cf",
            )?;
            let windage_cf = resolve_tracking_cf(
                windage_cf,
                saved_profile_data.as_ref().and_then(|p| p.windage_cf),
                "--windage-cf",
            )?;
            let twist_rate =
        ...
        Commands::Zero {
            ...
            // MBA-1358: zero has no --profile, so the CF comes only from the flag.
        ...
        // MBA-1358: tall-target helper — CF = actual measured travel / dialed travel.
        // Pure arithmetic over the same locked angular factors every dial column uses
        // (ballistics_engine::adjustment via drop_to_adjustment); no trajectory solve.
        Commands::TallTarget {
            dialed,
            measured,
            range,
        ...
        Commands::TrueVelocity {
            ...
            if uncertainty_requested {
                ...
                // MBA-1358: unlike the table-oriented single/multi displays, the
                // uncertainty report is a versioned schema document
                // (truing-uncertainty-v1) consumed for further analysis — its values
                // stay in TRUE (scope-corrected) angular units rather than being scaled
        ...
        Commands::ComeUps {
            ...
            // MBA-1358: elevation tracking CF — flag over profile field, validated here.
            let elevation_cf = resolve_tracking_cf(
                elevation_cf,
        ...
        Commands::Lead {
            ...
            let windage_cf = resolve_tracking_cf(
                windage_cf,
                profile_data.as_ref().and_then(|p| p.windage_cf),
                "--windage-cf",
            )?;
        ...
        Commands::PowerFactor {
            weight,
            velocity,
            organization,
            output,
        } => {
            handle_power_factor(weight, velocity, organization, cli.units, output)?;
        }
        ...
        Commands::WindCard {
            ...
            // MBA-1358: windage-axis tracking CF for the drift dial values.
            let windage_cf = resolve_tracking_cf(
                windage_cf,
                profile_data.as_ref().and_then(|p| p.windage_cf),
                "--windage-cf",
        ...
        Commands::RangeTable {
            ...
            let sight_offset_lateral_m = sight_offset
                ...
                .or_else(|| profile_data.as_ref().and_then(|p| p.sight_offset_lateral_m))
                .unwrap_or(0.0);
            // MBA-1358: per-axis tracking CFs, flag-over-profile.
            let elevation_cf = resolve_tracking_cf(
                elevation_cf,
                profile_data.as_ref().and_then(|p| p.elevation_cf),
                "--elevation-cf",
        ...
        Commands::Compare {
            ...
            let sight_offset_lateral_m = sight_offset
                .map(|v| sight_offset_display_to_metric(v, cli.units))
                .unwrap_or(0.0);
            // MBA-1358: compare has no single profile — CLI flags only, every load.
            let elevation_cf = resolve_tracking_cf(elevation_cf, None, "--elevation-cf")?;
            let windage_cf = resolve_tracking_cf(windage_cf, None, "--windage-cf")?;
            ...
            match action {
                ProfileAction::Save {
                    ...
                    let carried_sight_offset_lateral_m = existing_profile
                        .as_ref()
                        .and_then(|p| p.sight_offset_lateral_m);
                    // MBA-1358: like dsf_points/zero POI above — `profile save` cannot
                    // express tracking CFs (derive them with `tall-target`, store by hand
...
fn run_zero_range_calculation(
    ...
    match output {
        ...
        OutputFormat::Table => {
            ...
            // MBA-1358: the MOA/mrad representations are dial units and scale with the
            // CF, matching this path's own JSON/CSV zero_angle_moa/mrad; the degrees
...
/// 1 lbf in newtons, exact by definition (1 lbf = 1 lb-mass * standard gravity
/// 9.80665 m/s^2 = 4.4482216152605 N). Used only to display the SAAMI recoil impulse
/// (a genuine N*s / kg*m/s quantity) in pound-seconds for imperial output.
const LBF_TO_N: f64 = 4.4482216152605;

/// Free-recoil handler (MBA-1372). Converts the CLI's display-unit inputs (imperial:
/// grains for bullet/charge, pounds for firearm weight, fps; metric: grams, kilograms,
/// m/s) into `ballistics_engine::recoil`'s SI inputs, resolves the propellant gas
/// velocity from whichever override is given (`--gas-velocity` > `--gas-velocity-factor`
/// > `--firearm-type`), and renders energy/velocity/impulse back in display units.
#[allow(clippy::too_many_arguments)]
fn handle_recoil(
    bullet_weight: f64,
    charge_weight: f64,
    ...
    let bullet_mass_kg = match units {
        UnitSystem::Imperial => bullet_weight * GRAINS_TO_KG,
        UnitSystem::Metric => bullet_weight * 0.001, // grams -> kg
    };
    let charge_mass_kg = match units {
        UnitSystem::Imperial => charge_weight * GRAINS_TO_KG,
    ...
    match output {
        ...
        OutputFormat::Csv => {
            println!(
                "bullet_weight_{w},charge_weight_{w},velocity_{v},firearm_weight_{fw},gas_velocity_{v},recoil_velocity_{v},recoil_energy_{e},impulse_{i}",
                w = weight_unit,
                v = vel_unit_ascii(units),
                fw = match units { UnitSystem::Imperial => "lb", UnitSystem::Metric => "kg" },
                e = energy_unit_ascii(units),
                i = match units { UnitSystem::Imperial => "lbs", UnitSystem::Metric => "ns" },
            );
            println!(
                "{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.4}",
                bullet_weight,
        ...
        OutputFormat::Pdf => {
            return Err("PDF output is not supported for the recoil command".to_string());
        }
    }
    Ok(())
}

/// Power-factor handler (MBA-1372). PF is intrinsically defined in grains/fps by every
/// rulebook, so the input weight/velocity are converted to grains/fps regardless of
/// `--units` before calling into `ballistics_engine::power_factor`; the echoed
/// weight/velocity stay in the caller's own display units.
fn handle_power_factor(
    weight: f64,
    velocity: f64,
    organization: Option<String>,
    units: UnitSystem,
    ...
    let velocity_fps = match units {
        UnitSystem::Imperial => velocity,
        UnitSystem::Metric => velocity / FPS_TO_MPS,
    };
    ...
    match output {
        OutputFormat::Table => {
            println!("Power Factor");
            println!("============");
            println!("  Weight:              {:.1} {}", weight, weight_unit);
            println!("  Velocity:            {:.1} {}", velocity, vel_unit);
        ...
        OutputFormat::Pdf => {
            return Err("PDF output is not supported for the power-factor command".to_string());
        }
    }
    Ok(())
}
