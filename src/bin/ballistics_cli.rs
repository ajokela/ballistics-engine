...
fn calculate_trajectory_simple(
    ...
    let last = trajectory
        .last()
        .ok_or("Empty trajectory - no points generated")?;
...
fn run_monte_carlo(
    ...
    let std_range = (ranges.iter().map(|r| (r - mean_range).powi(2)).sum::<f64>() / n).sqrt();
    let std_height = (max_heights
        .iter()
        .map(|h| (h - mean_height).powi(2))
        .sum::<f64>()
        / n)
        .sqrt();
    let std_velocity = (impact_velocities
        .iter()
...
fn display_monte_carlo_results(
    result: MonteCarloResult,
    format: MonteCarloOutput,
    ...
    match format {
        MonteCarloOutput::Summary => {
            ...
            println!(
                "║ Mean:              {:>8.2} m/s        ║",
                result.mean_impact_velocity
            );
            println!(
                "║ Std Dev:           {:>8.2} m/s        ║",
                result.std_impact_velocity
            );
            println!("╠════════════════════════════════════════╣");
            println!("║ MAX HEIGHT                             ║");
            println!(
                "║ Mean:              {:>8.2} m          ║",
                result.mean_max_height
            );
            println!(
                "║ Std Dev:           {:>8.2} m          ║",
                result.std_max_height
            );
            println!("╚════════════════════════════════════════╝");
        }
...
fn display_cli_results(
    result: ballistics_engine::cli_api::TrajectoryResult,
    format: OutputFormat,
    full: bool,
) -> Result<(), Box<dyn Error>> {
    match format {
        ...
        OutputFormat::Table => {
            ...
            if full {
                ...
                println!(
                    "\nTrajectory Points (every {:.1}s):",
                    result.time_of_flight / 10.0
                );
                println!("┌──────────┬──────────┬──────────┬──────────┬──────────┐");
                println!("│ Time (s) │  X (m)   │  Y (m)   │  Z (m)   │ Vel(m/s) │");
                println!("├──────────┼──────────┼──────────┼──────────┼──────────┤");
