...
mod tests {
    ...
    fn test_apply_correction_g7() {
        ...
        for (caliber, weight, g7_bc, expected) in classification_cases {
            let got =
                cluster_bc.predict_cluster_for_drag_model(caliber, weight, g7_bc, DragModel::G7);
            assert_eq!(
                got, expected,
        ...
        let explicit_g1 =
            cluster_bc.apply_correction_for_drag_model(bc, 0.224, 77.0, 2800.0, DragModel::G1);
    ...
    fn g7_reference_ladder_is_inert_for_other_drag_models() {
        ...
        for drag_model in [
            ...
            let corrected =
                cluster_bc.apply_correction_for_drag_model(bc, 0.308, 168.0, 1100.0, drag_model);
            assert_eq!(
                corrected.to_bits(),
                bc.to_bits(),
