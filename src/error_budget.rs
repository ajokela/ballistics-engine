...
pub fn p_hit_bivariate(var_drop: f64, var_wind: f64, cov: f64, target: TargetGeometryV1) -> f64 {
    // A degenerate (non-positive-area) target can never be hit -- see doc comment's "A degenerate
    // target" section above. Checked before either covariance branch below, and unconditionally
    // (not only when the covariance also happens to be degenerate).
...
/// [`ErrorBudgetReportV1::method`] gains a `"_gl20_panelled_pm6sigma"` suffix and
...
mod tests {
    ...
    /// (Review, round 2 -- F2) There was no independent-accuracy oracle anywhere in this module
    /// for a circle with both variances positive -- every existing circle test either uses
    /// `p_hit_bivariate` as its own oracle, or (the degenerate-covariance tests) only ever
    /// evaluates the chord at `u = 0`, where `sqrt(r^2 - 0^2) == r` makes a bare `r` and the real
    /// chord formula indistinguishable. This closes that gap with the exact closed form for an
    /// isotropic, uncorrelated circular target: `(X, Y)` iid `N(0, sigma^2)` gives `R =
    /// sqrt(X^2+Y^2)` a Rayleigh distribution, `P(R <= r) = 1 - exp(-r^2 / (2*sigma^2))`.
    /// Measured agreement ~2e-5 (the quadrature's own residual near the chord's endpoint
    /// singularity -- see `p_hit_bivariate`'s doc); replacing the chord `sqrt(r^2 - u^2)` with a
    /// bare `r` (i.e. silently treating the circle as its own bounding square) is wrong by
    /// ~7.3e-2 on the first case below, three orders of magnitude past this test's tolerance.
    #[test]
    ...
    /// (Review, round 2 -- F2) A circle centred at the origin is rotationally symmetric, so its
    /// hit probability can only depend on the covariance's EIGENVALUES, not on which
    /// (drop, windage) basis it happens to be expressed in: `p_hit_bivariate(vd, vw, cov, Circle)`
    /// must equal `p_hit_bivariate(lambda1, lambda2, 0.0, Circle)`. This pins the chord formula
    /// AND the correlated (correlation-split) code path at once -- a bare-`r` chord mutation
    /// breaks rotational invariance directly, since it silently treats the circle as an
    /// axis-aligned square, which is NOT rotation invariant (measured diff up to ~1.1e-2 under
    /// that mutation, against ~2e-5 for the real chord). Reuses `Symmetric2`'s own
    ...
    /// (Review, round 2) `p_hit_gain_if_perfect` does NOT share `ellipse_area_reduction_m2`'s
    /// two-source degeneracy -- an earlier revision of this field's own doc comment claimed it
    /// did, which was simply wrong (see the corrected doc comment). Same exact fixture as
    /// `perfecting_a_source_never_lowers_p_hit` immediately above (the flagship
    /// `MuzzleVelocityMps` + `WindSpeed` pair), but checked against an INDEPENDENT oracle (direct
    /// `central_difference` calls, summed by hand) and required to discriminate by orders of
    /// magnitude, not merely "not exactly equal."
    #[test]
    fn p_hit_gain_if_perfect_discriminates_with_only_two_sources() {
        ...
        // Measured: ~0.4416 (WindSpeed) vs ~0.00008 (MuzzleVelocityMps) -- three and a half
        // orders of magnitude apart, not the tie ellipse_area_reduction_m2 would show here.
        assert!(
    ...
    /// (Review, round 2 -- F1) Proves the correlation-crossing panel split is load-bearing, not
    /// merely accuracy polish: this is exactly the realistic single-source rank-1 covariance
    /// shape documented on `p_hit_bivariate` (`sigma_drop = 18.5`, `sigma_windage = 6.0`, `rho`
    /// clamped to `-0.999_999`), at the specific target size (height 20x sigma_drop, width 1x
    /// sigma_windage) measured to be off by 0.275 WITHOUT the split.
    ///
    /// Independent oracle: as `|rho| -> 1`, windage collapses onto the deterministic line
    /// `windage = k * drop` (`k = rho * sigma_windage / sigma_drop`), so hitting the rectangle
    /// reduces to a ONE-DIMENSIONAL question about drop alone: `|drop| <= half_height` AND
    /// `|k * drop| <= half_width`, i.e. `|drop| <= min(half_height, half_width / |k|)`, giving
    /// `P = 2 * Phi(min(half_height, half_width / |k|) / sigma_drop) - 1`. This is a LIMIT (exact
    /// only at `rho = +-1` exactly, not the clamped `0.999_999`), so it is checked to a loose
    /// (2e-3) tolerance, not machine precision -- still two orders of magnitude tighter than the
    /// 0.275 error the mutation below produces.
    #[test]
    ...
    /// (Review, round 2 -- F4) The two monotonicity tests above check only 2-4 hand-picked
    /// points, all at `rho = 0` with isotropic sigma -- exactly the regime where the quadrature
    /// is simplest (a single panel, no correlation-crossing split) and least likely to expose a
    /// real non-monotonicity. Sweeps 20 log-spaced target sizes (both shapes) at `rho` in `{0.0,
    /// 0.9, 0.999_999}`, the same three correlations the accuracy tests elsewhere in this module
    /// exercise, so the near-rank-1 regime where panel boundaries move the most as the target
    /// grows is actually covered.
    ///
    /// The scale range (0.05x to 3x a baseline target size) is deliberately capped short of deep
    /// saturation (`p_hit` within a few `1e-8` of `1.0`): a target many sigma across on every
    /// axis genuinely can, and empirically does on this exact sweep at `rho = 0.999_999`, produce
    /// a sub-`1e-8` NON-monotone wiggle -- confirmed by extending this sweep's range during
    /// development and observing exactly that, which is why `p_hit_bivariate`'s own doc comment
    /// states monotonicity as a property of the true integral verified by test over a range, not
    /// a guarantee of the floating-point implementation at every conceivable input. The `1e-9`
    /// tolerance below is chosen to comfortably clear ordinary quadrature rounding while still
    /// catching a real regression (a transposition or dropped term would violate monotonicity by
    /// orders of magnitude more than `1e-9`, as the other tests in this module demonstrate).
    #[test]
    fn p_hit_is_monotone_in_target_size_across_a_sweep_including_near_rank_one_correlation() {
        ...
        let r0 = 0.25_f64;
        // 20 log-spaced scale factors from 0.05 to 3.0.
        ...
        for &rho in &[0.0_f64, 0.9, 0.999_999] {
            ...
            // Isotropic sigma for the circle sweep (a circle has one size parameter, not two).
    ...
    /// (Review, round 2 -- F6) A target with no positive area can never be hit, REGARDLESS of
    /// the covariance. Before this guard, `p_hit_bivariate(0.0, 0.0, 0.0, Rect { 0.0, 0.0 })`
    /// returned `1.0` (falling into the zero-total-variance branch above, which does not look at
    /// the target at all) -- technically defensible under a boundary-inclusive convention, but
    /// indistinguishable from a degenerate-target bug reading as total confidence. Checked with
    /// BOTH zero and positive declared variance, and for a negative dimension too (already
    /// clamped to zero elsewhere in this function, so should be treated identically to an
    /// explicit zero).
    #[test]
    ...
    /// `method`/`assumptions` field pin: the `"_gl20_panelled_pm6sigma"` suffix and the new
    /// hit-probability assumption must appear ONLY when a target is supplied, never when it is
    /// not (which would also break `the_report_declares_independence_and_linearity`'s
    /// exact-equality check on `method`). The suffix says "panelled" specifically (not the
    /// brief's own suggested `"_gl20_pm6sigma"`) so a machine consumer can tell this report used
    /// the piecewise quadrature, not a single 20-node call over the whole domain -- see
    /// `p_hit_bivariate`'s doc comment for why the difference matters.
    #[test]
