...
mod tests {
    ...
    fn erf_matches_libm_cross_check_over_a_range() {
        let cases = [
            ...
            (3.25, 0.9999956972205364),
            (3.5, 0.9999992569016276),
            (3.75, 0.9999998862727435),
            (4.0, 0.9999999845827421),
            (4.25, 0.9999999981494259),
            (4.5, 0.9999999998033839),
            (4.75, 0.999999999981515),
            (5.0, 0.9999999999984626),
            (5.25, 0.999999999999887),
            (5.5, 0.9999999999999927),
            (5.75, 0.9999999999999996),
            (6.0, 1.0),
        ];
        ...
        for (x, want) in cases {
            ...
            assert!(
                diff < 1e-12,
                "erf({x}) = {got:.17e} want {want:.17e} diff {diff:.3e}"
            );
        }
    ...
    fn normal_cdf_tail_is_real_not_flushed_to_zero() {
        ...
        assert!(
            far_tail > 0.0,
            "normal_cdf(-6.0) must be a real positive number, got {far_tail}"
        );
        ...
        for i in 1..=32 {
            ...
            assert!(
                v < prev,
                "erfc not strictly decreasing at x={x}: prev={prev:e} v={v:e}"
            );
            prev = v;
        }
    }
    ...
    fn erf_and_normal_cdf_are_monotonic_and_bounded_over_a_sweep() {
        let xs = [
            -5.0, -4.0, -3.0, -2.5, -2.0, -1.5, -1.0, -0.75, -0.5, -0.25, -0.1, 0.0, 0.1, 0.25,
        ...
        for x in xs {
            ...
            assert!(
                (0.0..=1.0).contains(&c),
                "normal_cdf({x}) = {c} out of [0, 1]"
            );
            assert!(
                e > prev_erf,
                "erf not strictly increasing at x={x}: prev={prev_erf} now={e}"
            );
            assert!(
                c > prev_cdf,
                "normal_cdf not strictly increasing at x={x}: prev={prev_cdf} now={c}"
            );
        ...
        for x in [10.0, 20.0, 50.0, 1.0e10] {
            ...
            assert_eq!(
                c, 1.0,
                "normal_cdf({x}) should have saturated to exactly 1.0"
            );
            assert!(
                (0.0..=1.0).contains(&nc),
                "normal_cdf(-{x}) = {nc} out of [0, 1]"
            );
            assert!(!nc.is_nan());
        }
    }
