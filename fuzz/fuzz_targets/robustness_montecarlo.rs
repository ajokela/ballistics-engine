...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(vsd) = ranged(&mut u, 0.0, 15.0) else {
        return;
    };
    let Ok(asd) = ranged(&mut u, 0.0, 0.005) else {
        return;
    };
    let Ok(bsd) = ranged(&mut u, 0.0, 0.03) else {
        return;
    };
    let Ok(wsd) = ranged(&mut u, 0.0, 5.0) else {
        return;
    };
    ...
    if let Ok(results) = run_monte_carlo_with_wind(inputs, WindConditions::default(), params) {
        ...
        for p in &results.impact_positions {
            ...
            assert!(
                is_sentinel || (p.x.is_finite() && p.y.is_finite() && p.z.is_finite()),
                "MC impact position not finite: {:?}",
                p
            );
        }
    }
